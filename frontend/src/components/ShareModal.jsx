...
export default function ShareModal({ file, session, onClose, onLinkCreated }) {
  ...
  async function handleCreateLink(e) {
    ...
    // 2. Fallback: Direct Supabase `share_links` table insert
    try {
      const shareRecord = {
        file_id: file.id,
        token: tokenStr,
      ...
      if (sbError) {
        console.error("Supabase share_links insert error:", sbError);
        ...
        <div className="modal-header flex-between">
          <div className="modal-title-box">
            ...
            <div>
              ...
              <p className="modal-subtitle">Configure access permissions for <strong>{file.filename}</strong> ({formatBytes(file.size_bytes)})</p>
            </div>
          </div>
